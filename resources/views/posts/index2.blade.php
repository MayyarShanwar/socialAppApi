...
@section('content')
    ...
    @php
        // Only pull a hero post on an unfiltered first page — otherwise the
        // "latest" framing stops making sense (e.g. sorted oldest-first).
        $showHero = $posts->isNotEmpty() && $posts->currentPage() === 1 && request('sort') !== 'asc';
        $hero = $showHero ? $posts->first() : null;
        $rest = $hero ? $posts->skip(1) : $posts;

        // Rough reading time, since we don't have any other metadata to show.
